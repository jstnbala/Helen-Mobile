...
class _OrderHistoryState extends State<OrderHistory> {
  ...
  Future<void> fetchOrders() async {
    ...
    // Check for cached completed orders
    ...
    if (cachedOrders != null && cacheTimestamp != null) {
      ...
      // If cached orders are still valid (less than 5 minutes old)
      if (DateTime.now().difference(cacheTime).inMinutes < 5) {
        orders = json.decode(cachedOrders); // Load from cache
        logger.i('Loaded orders from cache: $orders');
        if (mounted) setState(() { isLoading = false; }); // Update UI
        return;
      } else {
            ...
            child: ListView(
              ...
              children: [
                ...
                if (isLoading) ...[ // Show loading indicator while fetching
                  ...
                  const Center(child: Text('No completed orders available.')), // Message when no orders
                ] else ...[
                  // Display each order with Skeletonizer
